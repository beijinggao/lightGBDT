#ifndef LIGHTGBDT_DATASET_LOADER_H_
#define LIGHTGBDT_DATASET_LOADER_H_
...
public:
  ...
  LIGHTGBDT_EXPORT DatasetLoader(const IOConfig& io_config, const PredictFunction& predict_fun, int num_class, const char* filename);

  LIGHTGBDT_EXPORT ~DatasetLoader();

  LIGHTGBDT_EXPORT Dataset* LoadFromFile(const char* filename, int rank, int num_machines);

  LIGHTGBDT_EXPORT Dataset* LoadFromFile(const char* filename) {
    return LoadFromFile(filename, 0, 1);
  }

  LIGHTGBDT_EXPORT Dataset* LoadFromFileAlignWithOtherDataset(const char* filename, const Dataset* train_data);

  LIGHTGBDT_EXPORT Dataset* CostructFromSampleData(double** sample_values,
...
#endif // LIGHTGBDT_DATASET_LOADER_H_
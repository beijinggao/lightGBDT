#ifndef LIGHTGBDT_UTILS_LOG_H_
#define LIGHTGBDT_UTILS_LOG_H_
...
#define CHECK_NOTNULL(pointer) \
    if ((pointer) == nullptr)  \
        LightGBDT::Log::Fatal(#pointer "Can't be NUll at %s, line %d .\n", __FILE__, __LINE__);
#endif
...
#endif
        va_end(val);
        fprintf(stderr, "[LightGBDT] [Fatal] %s\n", str_buf);
        fflush(stderr);
...
// May be not good, but avoid to use an additional cpp file
    ...
    {
        static __declspec(thread) LogLevel level = LogLevel::Info;
        return level;
    }
#else
    static LogLevel &GetLevel()

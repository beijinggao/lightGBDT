#ifndef LIGHTGBDT_CONFIG_H_
#define LIGHTGBDT_CONFIG_H_

#include <LightGBDT/utils/common.h>
#include <LightGBDT/utils/log.h>

#include <LightGBDT/meta.h>
#include <LightGBDT/export.h>
...
public:
  ...
  std::string output_model = "LightGBDT_model.txt";
  std::string output_result = "LightGBDT_predict_result.txt";
  ...
  * Note: when using Index, it doesn't count the label index */
  ...
  LIGHTGBDT_EXPORT void Set(const std::unordered_map<std::string, std::string>& params) override;
};
